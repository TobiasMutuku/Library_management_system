...
$title = $author = $file_name = $description = $category = $image = $doc_type = "";
$posted_by = $user_name;
    ...
    if (empty($title_err) && empty($author_err) && empty($file_err) && empty($desc_err) && empty($category_err) && empty($item_exists_err) && empty($doc_type_err)) {
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                //
                $sql_check = "SELECT id, title FROM items WHERE title = ? AND author = ?";
                if ($stmt_check = $conn->prepare($sql_check)) {
                    ...
                    if ($result_check->num_rows > 0) {
                        ...
                        $update = mysqli_query($conn, "UPDATE requests SET status = 'replied', item_id = '$item_id', item_title = '$item_title' WHERE id = '$request_id'");
                        if ($update) {
...
<div class="container mt-3">
    <div class="container mt-5">
        <h2>Reply With Item</h2>
        <p>Please fill in the form below to send reply.</p>
