...
if (isset($_GET['q'])) {
    ...
    $books = get_books_by_category(10, $category);
} else {
    if ($field == "Other") {
        $books = get_all_books(30);
    } else {
        $books = get_books(100, $field);
    }
}
?>
    ...
    <div class="container mt-4" style="min-height: 58vh;">
        <div class="row">
            ...
            foreach ($books as $book) :
                $img_url = BASE_URL . '/uploads/images/' . $book['image'];
            ?>
                <div class="col-md-4 mb-4">
                    <div class="card">
                        <img src="./uploads/images/<?= $book['image'] ?>" class="card-img-top" alt="Cover Image" onerror="this.onerror=null;this.src='./uploads/images/default_cover.jpeg'; " width="auto" height="300">
                        <div class="card-body">
                            <h5 class="card-title"><?= $book['title'] ?></h5>
                            <p class="card-text"><?= substr($book['description'], 0, 30) ?>...</p>
                            <a href="inc/view_item.php?id=<?= $book['id'] ?>" class="btn btn-primary">Read More</a>
                        </div>
                        <div class="card-footer text-muted">
                            ...
                            echo "<small>Author: {$book['author']} <br>Posted On $date_str</small> <br> Category: {$book['category']}";
                            ?>
                        </div>
                    </div>
                </div>
